...
namespace PlayerResources
{
    ...
    public class ResourceManager : MonoBehaviour
    {
        ...
        private const float MORAL_DECREASE_AMOUNT = 0.014f;
        ...
        private void CheckIfPlayerDied()
        {
            ...
            if (hunger <= 0)
            {
                causeOfDeath = "You died of hunger, restart or return to menu";
                GameManager.Instance.ChangeGameState(GameManager.GameState.Died);
            }
            else if (thirst <= 0)
            {
                causeOfDeath = "You died of thirst, restart or return to menu";
                GameManager.Instance.ChangeGameState(GameManager.GameState.Died);
            }
            else if (raftHealth <= 0)
            {
                causeOfDeath = "The raft got destroyed, restart or return to menu";
                GameManager.Instance.ChangeGameState(GameManager.GameState.Died);
            }
        }
