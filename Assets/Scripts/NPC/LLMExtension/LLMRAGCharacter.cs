...
namespace NPC
{
    public class LLMRAGCharacter : LLMCharacter
    {
        ...
        {
            string prompt = await ConstructPrompt(npcData, query, state);

            string result = await Chat(prompt, callback, completionCallback, addToHistory);

            Debug.Log(grammarString);
            //npcData.AddConversationData(query, result);
        ...
        public async Task<string> ConstructPrompt(NPCDataLoader rag, string query, DetermineNPCState state)
        {            
