    ...
    public class NPCBase : MonoBehaviour, INPCMoveable
    {
        ...
        public void MoveNPC(Vector2 velocity)
        {
            if (!IsNPCAtTaskPosition())
            {
                ...
                CheckForFacingRightOrLeft(currentTaskPosition.position - transform.position);
                if(characterAnimationController != null)
                {
                    characterAnimationController.PlayAnimation(true);
                }
            ...
            else
            {
                ...
                if(characterAnimationController != null)
                {
                    characterAnimationController.PlayAnimation(false);
        ...
        public bool hasExecutedTask { get; set; } = false;
        private void PerformTask()
        {
            if (hasExecutedTask) return;

            performTaskAction.ExecuteTask();
